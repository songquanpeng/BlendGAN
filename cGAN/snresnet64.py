...
class SNResNetProjectionDiscriminator(nn.Module):
    ...
    def __init__(self, num_features=64, style_dim=0, activation=F.relu):
        ...
        self.l_y = utils.spectral_norm(nn.Linear(style_dim, num_features * 16))

        self._initialize()

    def _initialize(self):
        init.xavier_uniform_(self.l6.weight.data)
        # Cause we change l_y from Embedding to Linear, so we don't do that initialization anymore
    ...
    def forward(self, x, s):
        ...
        output = self.l6(h)
        output += torch.sum(self.l_y(s) * h, dim=1, keepdim=True)
        return output
